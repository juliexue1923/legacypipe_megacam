...
def main():
    ...
    parser.add_argument('--image-hdu', default=1, type=int, help='Read image data from the given HDU number')
    ...
    info('Reading first FITS header from image file...')
    ...
    if zpt is None:
        ...
        try:
          #  phot = ps1cat(ccdwcs=wcs, indexing='nested').get_stars(magrange=None)
            phot = GaiaCatalog(ccdwcs=wcs, indexing='nested', file_prefix='gaia').get_stars(magrange=None)
            info('Found', len(phot), 'GAIA stars in this image')
        except OSError as e:
            print('No GAIA stars found for this image -- outside the GAIA footprint, or in the Galactic plane?', e)
    if phot is not None:
        name = 'gaia'
        info('Cutting GAIA stars...')
        phot.cut(img.get_photometric_calibrator_cuts(name, phot))
        info(len(phot), 'GAIA stars passed cut to be used for calibration')
        if len(phot) == 0:
            phot = None
        else:
            # Convert to Legacy Survey mags
            info('Converting GAIA mags to the new camera...')
