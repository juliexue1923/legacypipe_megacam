...
class DELVECatalog(HealpixedCatalog):
    ...
    def __init__(self, file_prefix=None, indexing=None, ccdwcs=None, **kwargs):
        ...
        if indexing is None:
            indexing = os.getenv('DELVE_CAT_SCHEME', 'nested')
    ...
    def get_stars(self,magrange=None,band='g'):
        """Return the set of gaia stars on a given CCD with well-measured gri
        magnitudes. Optionally trim the stars to a desired r-band magnitude
        range.
        """
        cat = self.get_catalog_in_wcs(self.ccdwcs)
        print('Found {} good DELVE stars'.format(len(cat)))
        if magrange is not None:
            keep = np.where((getattr(cat,DELVECatalog.delveband[band])>magrange[0])*
                            (getattr(cat,DELVECatalog.delveband[band])<magrange[1]))[0]
            cat = cat[keep]
